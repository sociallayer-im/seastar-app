...
export default function CardEvent({ event, className, id, style, lang, highlight, additionalElement, isManager }: {
    ...
    const customHost = eventRoles?.find(r => r.role === 'custom_host')
    const groupHost = eventRoles?.find(r => r.role === 'group_host')
    const cohosts = eventRoles?.filter(r => r.role === 'co_host')
    const host = customHost?.display_name || groupHost?.display_name || event.owner?.nickname || event.owner?.name
    ...
    return <Link href={`/event/detail/${event.id}`}
        ...
        <div className="flex-1 mr-2 order-2 xs:order-1">
            ...
            <div className="flex flex-col text-xs sm:text-sm my-1">
                ...
                <div>hosted by {host}{!!cohosts && !!cohosts.length ? `, ${cohosts.map(c => c.display_name).join(', ')}` : ''}</div>
            </div>

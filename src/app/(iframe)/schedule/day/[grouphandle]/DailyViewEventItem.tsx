...
export default function DailyViewEventItem({event, timezone, lang}: {event: IframeSchedulePageDataEventDetail, timezone: string, lang: Dictionary}) {
    ...
    return <div className="w-full absolute shadow-sm rounded-sm p-2 overflow-hidden duration-300 hover:z-999 hover:scale-[1.01] cursor-pointer"
        onClick={() => showPopup(event.id, event.group_id, event.is_starred, lang)}
        style={event.style}>

...
export default async function EditFormPage(props: {params: Promise<{slug: string}>}) {
    const {lang} = await selectLang()
    const authToken = await getServerSideAuth()
    const currProfile = await getCurrProfile()
    if (!authToken || !currProfile) redirect(`/signin?return=/forms/${(await props.params).slug}/edit`)
    ...
    const form = await getForm({params: {slug: (await props.params).slug, authToken}, clientMode: CLIENT_MODE})

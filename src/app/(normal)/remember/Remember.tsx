...
export default function Remember({currProfile}: { lang: Dictionary, currProfile: ProfileDetail | null }) {
    ...
    async function handleCreateRememberVoucher(checkLogin = true) {
        ...
        const res = await createRememberVoucher({
            params: {
                authToken: authToken!,
                badgeClassId: rememberBadgeClassId
            },
    ...
    useEffect(() => {
        ...
        if (!!voucherId || !!isJoinedOtherVoucherId) {
            interval = setInterval(async () => {
                const res = await getJoinedRemember({
                    params: {voucherId: isJoinedOtherVoucherId! || voucherId!},
                    clientMode: CLIENT_MODE
                })
    ...
    useEffect(() => {
        if (joinedUser.length) {
            getUserPopupcitys({
                params: {ids: joinedUser.map((u => u.id))},
                clientMode: CLIENT_MODE
            })
    ...
    async function showRes(badge_item: BadgeClass) {
        openModal({
            content: (close: any) => {
    ...
    async function showCombine() {
        if (joinedUser.length < combineAmount) {
            toast({
                description: `You need ${combineAmount - joinedUser.length} more people to combine`,
                variant: 'warning'
            })
            return
        }
        ...
        if (totalPopUpCitys.length < popupCityAmount) {
            toast({
                description: `You need ${popupCityAmount - totalPopUpCitys.length} more popup-city to mint`,
                variant: 'warning'
            })
            return
        }
                ...
                return <div className={styles['show-combine']}>
                    <div className={styles['title']}>Are you sure you to combine a SBT with these people?</div>
                    {
                        joinedUser.map((u, i) => {
                            return <div className={styles['user-item']} key={i}>
                                <Avatar profile={u} size={16}/>
                                {displayProfileName(u)}
                            </div>
                        })
                    }
                    ...
                    <div className={styles['btns']}>
                        ...
                        <Button size={'sm'} onClick={() => {
                            close()
                            handleCombine()
                        }} variant={'special'}>Combine</Button>
                    </div>
                </div>
            }
        })
    ...
    return (<div>
            <div className={styles['wamo-merge-page']}>
                ...
                <div className={styles['center']}>
                    ...
                    <img className={styles['bg_2']} src="/images/remember_bg.png" alt=""/>
                    <i className={styles['bg_3']}/>
                    <div className={styles['page-header']}>
                        <div className={styles['left']}>
                            <a href={'/'}><img src="/images/logo_horizontal.svg" alt={''}/></a>
                        </div>
                    </div>
                    <div className={styles['main-contain']}>
                        ...
                        {!!user?.id && (voucherId || joinedTargetUser) &&
                            <div className={styles['code-bg']}>
                                ...
                                {
                                    !!joinedTargetUser && <>
                                        <Avatar profile={joinedTargetUser} size={40} className={styles['target-avatar']}/>
                                        <div
                                            className={styles['target-text']}>{displayProfileName(joinedTargetUser)} is
                                            combining
                                        </div>
                                    </>
                                }
                                ...
                                <div className={styles['icons']}>
                                    {
                                        totalPopUpCitys.map((u, i) => {
                                            return <Avatar profile={u as any} size={35} key={i}/>
                                        })
                                    }
                                    {
                        ...
                        <div className={styles['users']}>
                            {joinedUser.map((u, i) => {
                                return <div className={styles['user']} key={i}>
                                    <div>
                                        <Avatar profile={u} size={40} key={i}/>
                                        <div className={styles['name']}>{displayProfileName(u)}</div>
                                    </div>
                                    <div>
                                        ...
                                        <div className={styles['user-popup-cites']}>
                                            {
                                                joinedUserPopupcitys[u.id.toString()]?.groups.map((g, i) => {
                                                    return <Avatar profile={g as any} size={18} key={i}/>
                                                })
                                            }
                                        </div>
                                    </div>

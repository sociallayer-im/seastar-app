...
export default async function PromoCodeDetail(props: AsyncProps<PromoCodeDetailPageProps>) {
    const {lang} = await selectLang()
    const {couponDetail, currProfile, code, records} = await PromoCodeDetailData(await awaitProps(props))
    ...
    return <div className="min-h-[calc(100svh-48px)] w-full">
        <div className="page-width-sm min-h-[calc(100svh-48px) px-3 pb-12! pt-0">
            ...
            <div className="mb-4 flex-row-item-center justify-center">
                ...
                <div className="text-2xl flex-1 font-semibold w-[200px] shadow-sm rounded-lg text-center py-3">
                    {code}
                </div>
                <CopyText value={code}>
            ...
            {!records.length && <NoData/>}
            {
                records.map((record, i) => {
                    return <div key={i} className="flex-row-item-center border-b py-3 justify-between">

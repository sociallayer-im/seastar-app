import type { Dictionary } from "./index"

export const zh: Dictionary = {
  "Go": "继续",
  "Resend Code": "重新发送验证码",
  "or": "或",
  "Invalid email": "邮箱格式不正确",
  "User already exists": "该用户名已被使用",
  "Should be equal or longer than": "长度不得少于：",
  "Should be equal or shorter than": "长度不得超过：",
  "Ethereum Wallet": "以太坊钱包",
  "Google Auth": "Google 登录",
  "Sign in with WeChat": "微信登录",
  "WeChat sign-in failed, please try again": "微信登录失败，请重试",
  "No Ethereum wallet found in this browser": "未检测到以太坊钱包，请安装钱包扩展后重试",
  "Social Layer": "Social Layer",
  "Sign In": "登录",
  Email: "邮箱",
  "Check your inbox": "查看的收件箱",
  "Enter the code we sent to": "输入我们发送到",
  "to complete your account set-up": "的验证码完成您的帐户设置",
  Back: "返回",
  "Set a unique Social Layer username": "设置一个独特的 Social Layer 用户名",
  "Contain the English-language letters a-z and the digits 0-9":
    "包含英文字符 a-z 和数字 0-9",
  "Hyphens can also be used but it can not be used at the beginning and at the end":
    "可以使用连字符，但不能用于开头和结尾",
  "Hyphens cannot appear consecutively": "连字符不能连续出现",
  "Should be equal or longer than 6 characters": "应等于或长于 6 个字符",
  "Your username": "您的用户名",
  Confirm: "确认",
  "Bind Email": "绑定邮箱",
  "Signed in to your existing account": "该邮箱已有账号，已为你登录该账号。",
  "Please enter your email address so that you can log in and receive important notifications via email.":
    "请输入您的电子邮件地址，以便您可以通过电子邮件登录并接收重要通知。",
  "Your email": "您的邮箱",
  "Phone number": "手机号",
  "Invalid phone number": "请输入正确的中国大陆手机号",
  "Check your messages": "查看短信",
  "Bind Phone Number": "绑定手机号",
  "Please verify your mobile number to finish setting up your account.":
    "请验证您的手机号，以完成账号设置。",
  "Signed in to the account for this number": "该手机号已有账号，已为你登录该账号。",
  Continue: "继续",
  Skip: "跳过",
  "Event Schedule": "活动日程",
  Joined: "已加入",
  Filter: "筛选",
  Filters: "筛选",
  "Repeating Events": "重复活动",
  Tags: "标签",
  "Multi-day Events": "多天活动",
  Venues: "场地",
  Tracks: "Tracks",
  Cancel: "取消",
  "Show Events": "显示活动",
  "Email Address": "邮箱地址",
  "Wallet Address": "钱包地址",
  Close: "关闭",
  Copy: "复制",
  "Send a badge": "发送徽章",
  Followers: "被关注",
  Following: "关注",
  Groups: "群组",
  Events: "活动",
  Badges: "徽章",
  Sending: "发送中",
  Collected: "已收集",
  Created: "已创建",
  "Create a Group": "创建一个群组",
  "Edit Profile": "编辑个人资料",
  Avatar: "头像",
  "Upload Avatar": "上传头像",
  Nickname: "昵称",
  Location: "地点",
  Bio: "个人简介",
  "Social Links": "社交链接",
  Save: "保存",
  Edit: "编辑",
  Chat: "聊天",
  Votes: "投票",
  Members: "成员",
  "Group Profile": "群组资料",
  Admin: "管理员",
  "Please login first": "请先登录",
  "Email me when a member submits an event": "有成员创建活动时邮件通知我",
  "Failed to update notification setting": "更新通知设置失败",
  "View All Event": "查看所有活动",
  "Create an Event": "创建一个活动",
  Inviting: "邀请中",
  "Request to be Issuer": "请求成为发行者",
  Management: "管理",
  "Leave Group": "离开群组",
  "Search members...": "搜索成员...",
  "Edit Event": "编辑活动",
  "Create Event": "创建活动",
  "Event Track": "Track",
  "Event Name": "名称",
  "Event Description": "描述",
  "Event Note": "活动备注",
  "Display after confirming attendance": "确认参加后显示",
  "Upload Note Image": "上传备注图片",
  "Note image hint": "仅确认参加的用户可见，例如群聊二维码。",
  "Custom Location Name": "自定义地点名",
  "Input address or geo point eg. 4071.1,-74.06":
    "输入地址或者经纬度，例如：4071.1,-74.06",
  "Input a custom name for location": "输入自定义地点名",
  "When will it happen": "活动时间",
  "All Day Event": "全天活动",
  From: "开始",
  To: "结束",
  "Meeting URL": "会议链接",
  "Input meeting url": "输入会议链接，或直接粘贴腾讯会议邀请信息",
  "New Location": "新地点",
  Timeslots: "可用时间段",
  Overrides: "覆盖",
  Link: "链接",
  "Need Approval": "需要批准",
  "Select Venue": "选择地点",
  "Upload Cover": "上传封面",
  Host: "主持人",
  Profile: "个人",
  Group: "群组",
  "Venue Timeslots": "地点可用时间段",
  monday: "星期一",
  tuesday: "星期二",
  wednesday: "星期三",
  thursday: "星期四",
  friday: "星期五",
  saturday: "星期六",
  sunday: "星期日",
  Available: "可用",
  Unavailable: "不可用",
  "Due to the timeslot settings of the venue, only same-day events can be created":
    "由于场地的可用时间段设置，只能创建日内的活动",
  "Start time must be before end time": "开始时间必须在结束时间之前",
  "Available Date": "可用日期",
  "The selected time slot is occupied by another event at the current venue. Occupying event:":
    "所选时间段已被当前场地的另一个活动占用。占用活动：",
  "The date you selected is not available for the current venue due to the override settings":
    "由于覆盖设置，您选择的日期对于当前场地不可用",
  "The date you selected is not available for the current venue, requires manager permission":
    "您选择的日期对于当前场地不可用, 需要管理员权限",
  "The date you selected is not available for the current venue, requires member permission":
    "您选择的日期对于当前场地不可用, 需要成员权限",
  "The date you selected should be after the venue start date":
    "您选择的日期应在场地开始日期之后",
  "The date you selected should be before the venue end date":
    "您选择的日期应在场地结束日期之前",
  "The date you selected should be between the venue start date and end date":
    "您选择的日期应在场地开始日期和结束日期之间",
  "The date you selected is not available for the current venue due to the timeslot settings":
    "由于可用时间段设置，您选择的日期对于当前场地不可用",
  "The start time should be before the end time": "开始时间应在结束时间之前",
  "Only same-day events can be created": "只能创建日内的活动",
  "More Settings": "更多设置",
  "Event Badge": "活动徽章",
  "When an event participant checks in, he or she automatically receives a badge at the end of the event":
    "当活动参与者签到时，他/她将在活动结束时自动获得徽章",
  "Set a POAP badge for attendees": "为参与者设置 POAP 徽章",
  "Maximum participants": "最大参与者",
  Highlighted: "突出显示",
  "Select a highlight event, the event you created will display on the top of the day":
    "选择一个突出显示的活动，您创建的活动将显示在当天的顶部",
  "Require approval": "需要审核",
  "Users must apply and receive approval to attend in the event":
    "用户需要申请并获得批准才能参加活动",
  "Always required when an application form is enabled":
    "启用报名表单时必须开启",
  "Application Form": "报名表",
  "Collect info from applicants when they RSVP": "收集报名者的信息",
  "Question label": "问题标题",
  "Required": "必填",
  "Add question": "添加问题",
  "Remove Application Form": "移除申请表",
  "Are you sure you want to remove the application form? All questions will be cleared.": "确定要移除申请表吗？所有问题将被清除。",
  "Application form is not available for recurring events": "申请表不支持重复活动，请先取消重复设置。",
  "Submit Application": "提交申请",
  "Application submitted, pending approval": "申请已提交，等待审核",
  "This event requires approval. Your request will be reviewed by the organizer.": "该活动需要审核，您的申请将由组织者审查。",
  "Attending event": "已报名",
  "This field is required": "此项为必填",
  "View Application": "查看申请",
  "Edit Application": "编辑申请",
  "Update Application": "更新申请",
  "Application Answers": "申请内容",
  "No answers submitted": "未提交申请",
  "Close Event": "关闭活动报名",
  "People are no longer able to register for the event":
    "用户将无法继续报名该活动",
  "The maximum number of tags is 10": "标签的最大数量为 10",
  "Event Name is required": "活动名称是必需的",
  "Choose a badge from you created": "从您创建的徽章中选择",
  "Choose a badge from group": "从群组中选择徽章",
  "You will apply to use this venue": "您将申请使用此场地",
  "Invite Co-hosts": "邀请共同主持人",
  "Input event name": "输入活动名称",
  "Input name": "输入名称",
  "Input email to invite": "邀请邮箱",
  "Invite Speakers": "邀请演讲者",
  Share: "分享",
  "You have registered for the event.": "您已注册该活动。",
  "Your registration is under review.": "您的报名正在审核中，主办方确认后会通知您。",
  "Under Review": "审核中",
  "Welcome! To join the event, please register below.":
    "欢迎！要加入活动，请在下方注册。",
  "Add to Calendar": "添加到日历",
  Feedback: "反馈",
  "Join Event": "加入活动",
  "Check-In": "签到",
  "Check-In For Participants": "给参与者签到",
  "Check-In And Send POAP": "签到并发送 POAP",
  Tickets: "门票",
  "You have purchased the ticket": "您已购买门票",
  "Set a unique group name": "设置一个独特的群组名称",
  "My Events": "我的活动",
  Discover: "发现",
  Notifications: "通知",
  Registered: "已注册",
  Star: "星标",
  "Pending Requests": "待审批",
  "Co-hosting": "共同主持",
  "Member Management": "成员管理",
  "Manager Management": "管理员管理",
  "Transfer Owner": "转移所有者",
  "Remove Member": "移除成员",
  "Remove selected members": "移除选定的成员",
  "Select a recipient from the members": "从成员中选择接收者",
  "Create Badge": "创建徽章",
  "Sand Again": "再次发送",
  "Reason (Optional)": "原因（可选）",
  Next: "下一步",
  "Send Badge": "发送徽章",
  "Create Successfully": "创建成功",
  "Your badge have been created": "您的徽章已创建",
  "Leave empty to set the quantity as unlimited": "留空以将数量设置为无限",
  "send you a badge": "发送给您一个徽章",
  "Scan the QR Code": "扫描二维码",
  "has sent you a badge: []. Go and get it!":
    "已向您发送了一个徽章：[]. 去获取吧！",
  "Group Event Setting": "群组活动设置",
  Timezone: "时区",
  Banner: "横幅",
  "Event Permission": "活动权限",
  "Event Approval Notification": "活动审批通知",
  "Who can submit events for review": "谁可以申请发布活动（需审批）",
  "Disabled": "关闭",
  "venues are the default locations that creators can choose for their events.":
    "场地是创建者可以为其活动选择的默认位置",
  "Create a Venue": "创建一个场地",
  "Create a Track": "创建一个 Track",
  "Event Tags": "活动标签",
  "Event tags that creators can choose for their events.":
    "创建者可以为其活动选择的活动标签",
  "Create a Tag": "创建一个标签",
  "Default time zone for group event, but you can still change it when creating the event. If keep it blank, the default time zone will follow the operating system.":
    "群组活动的默认时区，但您仍然可以在创建活动时更改它。如果保留为空白，则默认时区将遵循操作系统。",
  Image: "图片",
  "Who can create events": "谁可以创建活动",
  "Who can join events": "谁可以加入活动",
  "Who can view events": "谁可以查看活动",
  "Require approval for events": "活动发起需要审批",
  "Events created by non-managers stay pending until a manager approves them": "非管理员发起的活动将处于待审核状态，需要管理员通过后才会发布",
  "Edit Venue": "编辑场地",
  "Name of venue": "场地名称",
  "Default location": "默认位置",
  "Description (Optional)": "描述（可选）",
  "Link (Optional)": "链接（可选）",
  "Available Date (Optional)": "可用日期（可选）",
  Visibility: "可见性",
  "Venue Capacity (Optional)": "场地容量（可选）",
  "Booking Window (Optional)": "可预订时间范围（可选）",
  "Restrict which dates this venue can be booked. Leave either side blank for no limit.": "限制该场地可被预订的日期范围，留空表示不限制。",
  "Require Approval (Optional)": "需要批准（可选）",
  "Membership card": "会员卡",
  "Membership card hint": "购买后获得所选时长的会员权限，在有效期内再次购买会在剩余时间基础上叠加。",
  "Membership duration": "会员有效期",
  "7 days": "7天",
  "30 days": "30天",
  "365 days": "365天",
  "Custom days": "自定义天数",
  "Opening Hours": "营业时间",
  "Add dates when your availability changes from your daily hours.":
    "添加日期，以便您的可用性从每日小时数更改。",
  "Edit Track": "编辑 Track",
  "Name of track": "Track 名称",
  "Icon (optional)": "图标（可选）",
  "Display on the schedule page": "在日程页面上显示",
  "Manager Setting": "管理员设置",
  "Generate Promo Code": "生成优惠码",
  "Valid Date": "有效日期",
  Generate: "生成",
  "Can be used": "可使用",
  Times: "次",
  "Promo Code": "优惠码",
  "You have generated a promo code": "您已生成一个优惠码",
  Code: "代码",
  "Remaining uses": "剩余使用次数",
  "Ticket Setting": "门票设置",
  "Does not Repeat": "不重复活动",
  "Repeat period": "重复周期",
  "Repeat Setting": "重复设置",
  "Repeat Times": "重复次数",
  "Event Times Preview": "活动时间预览",
  "Every Day": "每天",
  "Every Week": "每周",
  "Every Month": "每月",
  "Repeat every [1], [2] times": "每[1]重复，[2]次",
  times: "次",
  repeat: "重复",
  "Add a Ticket Type": "添加门票类型",
  Ticket: "门票",
  "Name of Tickets": "门票名称",
  "Ticket description": "门票描述",
  Price: "价格",
  Free: "免费",
  Payment: "支付",
  "Ticket amount": "门票数量",
  "No limit": "无限制",
  Limit: "限制",
  "Ticket sales end time": "门票销售结束时间",
  Qualification: "资格",
  "People possessing the badge you select have the privilege to make payments at this price.":
    "拥有您选择的徽章的人可以购买门票",
  "Select a Badge": "选择一个徽章",
  "Receiving wallet": "接收钱包",
  "Create a new badge": "创建一个新徽章",
  "Are you sure you want to remove the selected member?":
    "您确定要移除选定的成员吗？",
  "Add a manager": "添加管理员",
  "The group manager has the following permissions:":
    "群组管理员具有以下权限：",
  "1. Send the created badge to others": "1. 将创建的徽章发送给其他人",
  "2. Manage the badge of group": "2. 管理群组的徽章",
  "3. Edit the group profile": "3. 编辑群组资料",
  "4. Remove group member": "4. 移除群组成员",
  "Remove manager": "移除管理员",
  "Do you want to remove this manager?": "您要移除此管理员吗？",
  "Selected a member in group": "在群组中选择一个成员",
  Add: "添加",
  "Freeze Group": "冻结群组",
  "Are you sure you want to freeze this group? This action cannot be undone.":
    "您确定要冻结此群组吗？此操作无法撤消。",
  "Are you sure you want to transfer the ownership of this group to this member?":
    "您确定要将此群组的所有权转让给此成员吗",
  "Are you sure you want to leave the group?": "您确定要离开群组吗？",
  "Invite Member": "邀请成员",
  "Member until": "会员有效期至",
  "Invite Message (Optional)": "邀请消息（可选）",
  "Create Invite Link": "创建邀请链接",
  "Users join the group via a link":
    "用户通过链接加入群组",
  Receivers: "接收者",
  Role: "角色",
  Manager: "管理员",
  Member: "成员",
  Issuer: "颁发者",
  "Invite you to become [1] [2]": "邀请您成为 [1] 的 [2]",
  "Input the username or email of the people can receive the invite.":
    "输入接收邀请者的用户名、电子邮件或手机号。",
  "Download CSV Template": "下载 CSV 模板",
  "Import From CSV File": "从 CSV 文件导入",
  "Send Invite": "发送邀请",
  "Group Invite": "群组邀请",
  "The invitation has been sent!": "邀请已发送！",
  "Copy Link": "复制链接",
  "Back to Group": "返回群组",
  "Invite Detail": "邀请详情",
  "Invite Message": "邀请消息",
  Time: "时间",
  Reject: "拒绝",
  Rejected: "已拒绝",
  "Are you sure you want to reject this participant?": "确定要拒绝该参与者吗？",
  Accept: "接受",
  Send: "发送",
  Later: "稍后",
  "Select receivers": "选择接收者",
  "Badge amount": "徽章数量",
  Reason: "颁发理由",
  "Create Time": "创建时间",
  Receiver: "接收者",
  Creator: "创建者",
  "Only the creator and the recipient of the badge can view the details":
    "只有徽章的创建者和接收者可以查看详情",
  "Voucher Detail": "凭证详情",
  "You have accepted": "您已接收",
  "Input username or email": "输入用户名或邮箱",
  "Basic Badge": "基础徽章",
  "Basic badge, evaluation of others": "基础徽章，评价他人",
  "Privacy Badge": "隐私徽章",
  "Only receivers can see the badge detail": "只有接收者可以看到徽章详情",
  "Badge Image": "徽章图片",
  "Badge Name": "徽章名称",
  "Naming your badge": "为您的徽章命名",
  "Input reason": "输入原因",
  Person: "个人",
  "Send a badge to [1]": "给[1]发送徽章",
  "Choose a image for badge": "选择徽章图片",
  "Upload Image": "上传图片",
  Public: "公开",
  "Go to": "前往",
  "Pop-up Cities": "快闪城市",
  "See all Pop-up Cities events": "查看所有快闪城市",
  "View events": "查看活动",
  "See all Communities": "查看所有社区",
  Attended: "已参加",
  Hosting: "我发起的",
  Starred: "已关注",
  "Send a Badge": "发送徽章",
  "Sign in to participate in a fun event": "登录以参加有趣的活动",
  "Add iCal Subscription": "添加 iCal 订阅",
  "Add events to your calendar to stay updated.":
    "将活动添加到您的日历以保持更新。",
  "System Calendar": "System Calendar",
  Upcoming: "即将到来",
  Past: "已结束",
  Private: "私密",
  Unlisted: "不公开列出",
  Reset: "重置",
  "All Time": "全部时间",
  Today: "今天",
  Week: "本周",
  Month: "本月",
  "Attend event to view notes": "参加活动以查看笔记",
  "All Tags": "所有标签",
  "All Tracks": "所有 Track",
  Seats: "座位",
  Display: "展示",
  "Normal Event": "普通活动",
  "Listed publicly. Who can register still follows the community's registration setting.":
    "公开展示，能否报名仍然遵循社区的报名设置。",
  "Open Registration": "开放报名",
  "Anyone can register for this event, even though the community normally requires membership to register.":
    "即使社区要求必须是成员才能报名，这个活动仍然对所有人开放报名。",
  "Unlisted Event": "不公开列出的活动",
  "Not shown in public listings. Only reachable by direct link, and visible to community members on the My Events page.":
    '不会出现在公开列表中，只能通过链接访问，社区成员可以在"我的活动"页面看到。',
  "You can set up POAP after the event is created.":
    "您可以在创建活动后设置 POAP。",
  "Share Event": "分享活动",
  "Join this event on Social Layer": "在 Social Layer 上参加这个活动",
  "Tap ··· in the top right to share to WeChat": "点击右上角的 ··· 分享到微信",
  "Scan the code": "扫描二维码",
  "and attend the event": "并参加活动",
  "Save Image": "保存图片",
  "Event Detail": "查看活动详情",
  Copied: "已复制",
  "Cancel Event": "取消活动",
  "Event Canceled": "活动已取消",
  "Are you sure you want to cancel this event?": "您确定要取消此活动吗？",
  "Check In": "签到",
  "Accept to join": "批准加入",
  "Cancel Participation": "取消参与",
  "Are you sure you want to cancel your participation?": "您确定要取消参与吗？",
  "Participation cancelled": "参与已取消",
  "Are you sure you want to check in for this participant?":
    "您确定要为此参与者签到吗？",
  "Check in successful": "签到成功",
  Checked: "已签到",
  "Download the list of all participants": "下载所有参与者的列表",
  "Download all form submissions": "下载所有报名表单提交",
  Participants: "参与者",
  "Scan the Participant's QR Code": "扫描参与者的二维码",
  "Online event, please check in for participants below.":
    "在线活动，请在下方为参与者签到。",
  "Send POAP": "发送 POAP",
  "Do you want to send POAP to participants checked in ?":
    "您要发送 POAP 给已签到的参与者吗？",
  "POAP Sent": "POAP 已发送",
  "Upcoming Events": "个即将开始的活动",
  "Create a Marker": "创建一个标记",
  "Browse on Map": "在地图上浏览",
  "All Markers": "所有标记",
  Title: "标题",
  "Input location": "输入位置",
  "Input marker title": "输入标记标题",
  Category: "类别",
  "Input marker description": "输入标记描述",
  "Image (Optional)": "图片（可选）",
  Marker: "标记",
  "Edit Marker": "编辑标记",
  Remove: "移除",
  day: "天",
  month: "月",
  week: "周",
  year: "年",
  every: "每",
  "View map": "查看地图",
  "Select the scope of application": "选择应用范围",
  "Only this event": "仅此活动",
  "All recurring events": "所有重复活动",
  "This event and following events": "此活动和后续活动",
  "Save successful": "保存成功",
  "Save failed": "保存失败",
  "Enter a tag": "输入标签",
  "Remove Track": "移除 Track",
  "Are you sure you want to remove this track?": "您确定要移除此 Track 吗？",
  Removed: "已移除",
  "Remove failed": "移除失败",
  Select: "选择",
  "Create Track": "创建 Track",
  Create: "创建",
  "Create successful": "创建成功",
  "Create failed": "创建失败",
  "Remove Venue": "移除场地",
  "Are you sure you want to remove this venue?": "您确定要移除此场地吗？",
  "Edit Override": "编辑覆盖",
  "Create Override": "创建覆盖",
  "Override Date": "覆盖日期",
  "Override Time": "覆盖时间",
  "Mark Unavailable": "标记不可用",
  "Add new Override": "添加新覆盖",
  "Events, Groups, Badges, Users": "活动，群组，徽章，用户",
  Event: "活动",
  User: "用户",
  Badge: "徽章",
  "Please input username": "请输入用户名",
  'Username cannot start with "-"': '用户名不能以"-"开头',
  'Username cannot end with "-"': '用户名不能以"-"结尾',
  "Username contains invalid character": "用户名包含无效字符",
  "The minimum length of username is ": "用户名的最小长度为",
  "The maximum length of username is ": "用户名的最大长度为",
  "Contain the English-language letters a-z, the digits 0-9 and hyphens":
    "包含英文字符 a-z、数字 0-9 和连字符",
  "This name is reserved": "该名称被站点占用，请换一个",
  "Group handle": "群组 handle",
  "Do you want to create a group with this name ?":
    "您要使用这个名创建一个群组吗？",
  "Event Home": "前往社区主页",
  "Events of [{1}]": "「{1}」的活动",
  Content: "内容",
  "Ticket Detail": "门票详情",
  "Ticket Type": "门票类型",
  "Badge Needed": "所需徽章",
  "Payment Methods": "支付方式",
  Pay: "支付",
  "Copy Payment Link": "复制支付链接",
  "Your Tickets": "你的门票",
  "Purchase for Free": "免费购买",
  "Purchase Successful": "购买成功",
  "Your payment transaction": "你的支付交易",
  "Sold Out": "售罄",
  "Stop Selling": "停止销售",
  "You can add a badge after creating an event": "您可以在创建活动后添加徽章",
  "Generated Promo Code": "生成优惠码",
  "Promo Code Detail": "优惠码详情",
  "Input comment": "输入评论",
  "Sign in to send a comment": "登录以发送评论",
  "Send a Comment": "发送评论",
  Comments: "评论",
  "Comment sent": "评论已发送",
  "Amount Off": "减免金额",
  "Percentage Off": "折扣百分比",
  "Usage Record": "使用记录",
  "Input discount": "输入折扣",
  Unlimited: "无限制",
  Apply: "应用",
  "Promo Code Required": "需要优惠码",
  Settings: "设置",
  "Create Group": "创建群组",
  Notification: "消息通知",
  "Sign Out": "退出登录",
  Invites: "邀请",
  "Send you a badge": "发送给您一个徽章",
  "Registration for the event, upon completion, will be rewarded with POAP*1":
    "活动注册，完成后将获得 POAP*1",
  Discount: "折扣",
  Total: "总计",
  Pending: "待审核",
  Canceled: "已取消",
  Closed: "已关闭",
  Ongoing: "进行中",
  Publish: "发布",
  "Approve Event": "批准活动",
  "Are you sure to approve this event ?": "您确定要批准此活动吗？",
  "Reject Event": "拒绝活动",
  "Are you sure you want to reject this event?": "您确定要拒绝此活动吗？",
  "Seating Arrangement Style": "座位排列风格",
  "AV Needed": "AV 需求",
  "Are you sure you want to cancel these events?": "您确定要取消这些活动吗？",
  "Submit your feedback for this event": "提交您对此活动的反馈",
  "Copy Address": "复制地址",
  Speaker: "演讲者",
  "Co-Host": "共同发起",
  "This event is only for {}": "此活动仅限{}。",
  members: "成员",
  managers: "管理员",
  Everyone: "所有人",
  "Member, Manager, Owner": "成员，管理员，群组所有者",
  "Manager, Owner": "管理员，群组所有者",
  "Badge Detail": "徽章详情",
  "Issuance Time": "颁发时间",
  "Online Meeting": "在线会议",
  "Tencent Meeting": "腾讯会议",
  Orders: "订单",
  "Download the list of order": "下载订单列表",
  "Select a group": "选择一个群组",
  "Use this group as a community": "将此群组用作社区",
  "Please select a group": "请选择一个群组",
  "Choose a name for your Popup-City": "为您的快闪城市选择一个名称",
  "You can create events with your buddies.": "您可以与您的朋友创建活动。",
  "More Details": "更多详情",
  "Set the detailed information for your Popup-City":
    "设置您的快闪城市的详细信息",
  Cover: "封面",
  Duration: "持续时间",
  "City Name": "城市名称",
  "Please upload a cover image": "请上传封面图片",
  "Please input the city name": "请输入城市名称",
  "Please input the duration": "请输入持续时间",
  "Who can view/create/attend events?": "谁可以查看/创建/加入活动？",
  "Please select the timezone": "请选择时区",
  View: "查看",
  Attend: "参加",
  "Invite Managers": "邀请管理员",
  "Create Popup-City": "创建快闪城市",
  "You have create a Popup-City": "创建了快闪城市",
  "Set Event Permission": "设置活动权限",
  "View Event Home Page": "浏览活动主页",
  "Website (optional)": "网站（可选）",
  "Edit Popup-City": "编辑快闪城市",
  "Popup-City Name": "快闪城市名称",
  "Please input Popup-City name": "请输入快闪城市名称",
  "Enabled": "已启用",
  "Not set": "未设置",
  "Featured Image": "推荐图片",
  "Set a city and date range to run this group as a popup city — it will then be eligible to appear in Popup City discovery. Leave the date range blank to keep it a regular community.":
    "设置城市和日期范围，将该群组作为快闪城市运行——设置后将有资格出现在快闪城市发现页中。将日期范围留空则保持为普通社区。",
  "Input event URL": "输入活动链接",
  "Support URL of Luma": "支持 Luma 的 URL",
  "Load the Event URL": "加载活动 URL",
  "External Event": "外部活动",
  "External Event Imported Successfully": "外部活动导入成功",
  "Importing external event details will disregard ticket information and online meeting addresses. Please manually edit these details.":
    "导入外部活动详情将忽略门票信息和在线会议地址。请手动编辑这些详情。",
  "Import External Event": "导入外部活动",
  "Want to create your own Pop-up City?": "想要创建您自己的快闪城市吗？",
  "Want to create your own Group?": "想要创建您自己的社区吗？",
  "Start now and let more people freely organize and participate in your exciting events!":
    "立即开始，让更多人自由组织和参与您的精彩活动！",
  "Create Now": "现在创建",
  "Sign In to Attend": "登录以参加",
  "Buy Tickets": "购买门票",
  "View Details": "查看详情",
  All: "全部",
  "Share Schedule": "分享日程",
  "Schedule Link": "日程链接",
  "Embed Code": "嵌入代码",
  "All Venues": "所有场地",
  "Time Range": "时间范围",
  "Reset Filter": "重置筛选器",
  "Can not change the repeat setting after the event is created":
    "无法在活动创建后更改重复设置",
  "After {date}": "{date} 之后",
  "Before {date}": "{date} 之前",
  "Venue List": "场地列表",
  "Venue Detail": "场地详情",
  "Cover/Poster (Optional)": "封面/海报（可选）",
  "Add a photo": "添加照片",
  Amenities: "设施",
  Capacity: "容量",
  "When set, this appears as the event host.":
    "如果设置，这将显示为活动主持人。",
  "Custom Host": "自定义主持人",
  "Join Event(RSVP)": "加入活动（RSVP）",
  Programs: "项目",
  Program: "项目",
  "All Programs": "所有项目",
  "Event Programs": "活动项目",
  "Create a Program": "创建一个项目",
  "Create Program": "创建项目",
  "Edit Program": "编辑项目",
  "Name of program": "项目名称",
  "Everyone can view events in the program": "所有人都可以查看项目中的活动",
  "Only program members or group managers can view events in the program.":
    "只有项目成员或群组管理员可以查看项目中的活动",
  "Remove Program": "移除项目",
  "Are you sure you want to remove this program?": "您确定要移除此项目吗？",
  'Communities': '社区',
  'We value your feedback!': '我们重视您的反馈！',
  'About us': '关于我们',
  'Contact us': '联系我们',
  'Remember': 'Remember',
  'My Badge': '我的徽章',
  'My Groups': '我的群组',
  'Setting': '设置',
  'About': '关于',
  'What is Badge': '什么是徽章',
  'Development Doc': '开发文档',
  'Unhighlighted': '取消高亮',
  'Where are you based?': '您位于哪个城市？',
  'Export Event Data': '导出活动数据',
  'Export': '导出',
  'Highlight': '高亮',
  'Export Event Participant Data': '导出活动参与者数据',
  'Specify Programs (Optional)': '指定项目 （可选）',
  'Only program members can see and select this venue to create events. You can manager program members in the program setting page. ': '只有项目成员可以查看和选择此场地以创建活动。您可以在项目设置页面管理项目成员。',
  'Kind(Optional)': '类型（可选）',
  'Select Kind': '选择类型',
  'Kind': '类型',
  'Highlighted Events': '置顶活动',
  'Cancelled': '已取消',
  'You have accepted the invite': '您已接受邀请',
  'The other party has successfully joined the group': '对方已成功加入群组',
  'Order awaiting payment': '你在该活动已有一笔待支付或待确认的订单，请先完成它，不要重新下单。',
  'Group ticket event': '群组门票活动',
  'Group ticket event intro': '该活动下的门票将授予群组成员资格，而不只是参加这一场活动。每个群组只能有一个。',
  'Group ticket event exists': '该群组已存在群组门票活动。',
  'Registered at': '报名于',
  'Checked in at': '签到时间',
  'Stripe Payment Keys': 'Stripe 收款密钥',
  'Stripe keys intro': '添加你的 Stripe secret key 即可用银行卡售票，收入直接进入你的 Stripe 账户。建议使用带 charge/refund 权限的受限密钥（rk_live_…）而非完整密钥。',
  'Key Name': '密钥名称',
  'Secret Key': 'Secret Key',
  'Secret key hint': 'Stripe Dashboard → Developers → API keys → Secret key（点 Reveal 显示）。不是 publishable key（pk_…），后者无法收款。',
  'Publishable key error': '这是 publishable key（pk_…），无法用于收款。请使用 Secret key（sk_…）或受限密钥（rk_…）。',
  'Add Key': '添加密钥',
  'Remove Key': '删除密钥',
  'TEST MODE': '测试模式',
  'Delayed confirmation': '确认延迟',
  'Card Payment (Stripe)': '银行卡支付（Stripe）',
  'Add card payment': '添加银行卡支付',
  'Select a Stripe key': '选择 Stripe 密钥',
  'Card price minimum': '银行卡支付价格不能低于 $4，更低金额请使用加密货币支付',
  'Stripe key required': '请为银行卡支付方式选择一个 Stripe 密钥',
  'Refund': '退款',
  'Refund this order': '全额退款该订单？座位将被释放，买家将通过原支付方式收到退款。',
  'Refund submitted': '退款已提交，稍后完成',
  'No orders yet': '还没有订单',
  'Total revenue': '总收入',
  'Available to withdraw': '可提现金额',
  'WeChat Pay charges a {wechat_pct}% fee and sola charges a {sola_pct}% fee; the rest is available to withdraw.':
    '微信支付收取 {wechat_pct}% 手续费，sola 收取 {sola_pct}% 手续费，剩余部分可以提现。',
  'WeChat Pay Withdrawals': '微信支付提现',
  'View your withdrawal records and request a withdrawal': '查看提现记录并申请提现',
  'WeChat Pay withdrawal intro':
    '微信支付的活动收入统一进入 sola 的商户账户，因此需要人工线下结算给组织者。每次提现会提取全部可提现余额，工作人员处理后会在这里更新状态。',
  'Withdraw All': '全部提现',
  'Confirm Withdrawal': '确认提现',
  'Withdrawal requested': '已提交提现申请，工作人员会线下处理并在此更新状态',
  'Bank Name': '开户行',
  'Bank Account Number': '银行卡号',
  'Bank Account Holder Name': '持卡人姓名',
  'Cumulative withdrawn': '累计提现金额',
  'Withdrawal Records': '提现记录',
  'No withdrawal records yet': '还没有提现记录',
  'Prev': '上一页',
  'Note for the group\'s managers (optional)': '给社区管理员的备注（可选）',
  'Reason for rejecting (shown to the group\'s managers)': '拒绝原因（会展示给社区管理员）',
  'Admin: WeChat Pay Withdrawals': '管理后台：微信支付提现',
  'Requested by': '申请人',
  'Processed by': '处理人',
  'Mark Settled': '标记为已结算',
  'Mark this withdrawal as settled? Only do this after the bank transfer has actually been sent.':
    '确认将这笔提现标记为已结算？请在实际完成银行转账后再操作。',
  'Reject this withdrawal? The amount becomes available to request again.':
    '确认拒绝这笔提现？金额将重新变为可提现状态。',
  'No groups to manage withdrawals for': '没有可管理提现的社区',
  'This balance is shared with the group\'s other managers.': '这笔余额由该社区的所有管理员共享。',
  'Awaiting settlement': '待结算',
  'Settled': '已结算',
  'Failed to load': '加载失败',
  'Retry': '重试',
  'Payment processing': '已收到支付，正在确认你的票…',
  'Payment confirmed': '支付成功！你的票已就绪。',
  'Payment cancelled': '支付已取消，未支付的预留将在稍后过期。',
  'WeChat Pay': '微信支付',
  'Add WeChat payment': '添加微信支付',
  'WeChat Pay intro': '买家在微信内完成支付。款项由平台商户号收取后与你另行结算，你无需自己申请商户号。',
  'WeChat price minimum': '微信支付价格不能低于 ¥0.01',
  'Open in WeChat to pay': '微信支付需要在手机微信中完成，请用手机微信打开此页面后再支付。',
  'Payment failed, please try again': '支付未完成。如果已经扣款，你的票会稍后出现，请不要重复支付。',
  'Payment confirming slowly': '已收到支付，确认比平时慢一些。你的票稍后会出现在本页，请不要重复支付。',
  'Order total': '订单金额',
  'Refundable remaining': '可退余额',
  'Refund amount': '退款金额',
  'Refund reason (optional)': '退款原因（选填）',
  'Invalid refund amount': '请输入大于 0 且不超过可退余额的金额',
  'Full refund releases the seat': '全额退款会释放座位、退还优惠券用量，并取消该参与者。',
  'Partial refund keeps the seat': '部分退款只退钱，参与者保留座位。',
  'Order placed': '下单',
  'Order paid': '支付成功',
  'Payment started': '发起支付',
  'Payment retried': '重新发起支付',
  'Payment notice rejected': '支付通知被拒绝',
  'Seat released': '座位已释放',
  'Dispute opened': '发起争议',
  'Dispute won': '争议胜诉',
  'Dispute lost': '争议败诉',
  'Coupon used': '使用优惠券',
  'Ticket granted': '票已发放',
  'Status changed': '状态变更',
  'estimated': '推算',
  'Reconstructed entry': '由时间戳推算，并非当时记录，时间点可能不精确。',
  'Refund requested': '发起退款',
  'Refunded': '已退款',
  'Refund failed': '退款失败',
  'Full refund': '全额',
  'Partial refund': '部分',
  'Order expired': '订单超时关闭',
  'Order cancelled': '订单已取消',
  'Awaiting payment': '待支付',
  'Paid': '已支付',
  'Partially refunded': '部分退款',
  'Disputed': '争议中',
  'Expired': '已超时',
  'Ticket approval hint': '使用此票报名需要主办方审核通过后才算报名成功。',
  'Members Only (Optional)': '仅限成员（可选）',
  'Members only ticket hint': '只有所选社区的成员才能领取此票。不选择则对所有人开放。',
  'Members Only': '仅限成员',
  'You are a member': '你是成员',
  'Members only ticket notice': '此票仅限以下社区的成员领取。',

  // OAuth provider
  'Authorization failed': '授权失败',
  'Provided by': '提供方',
  'an unknown developer': '未知开发者',
  'This application is provided by a third-party developer and has not been reviewed by Social Layer.': '该应用由第三方开发者提供，Social Layer 未审核。',
  'This application will be able to access:': '该应用将可以访问：',
  'You will be returned to': '授权后将跳转至',
  'Deny': '拒绝',
  'Authorize': '授权',
  'You can revoke this access at any time in Settings.': '你可以随时在设置中撤销此授权。',
  'Developer': '开发者',
  'OAuth Applications': 'OAuth 应用',
  'Create Application': '创建应用',
  'No applications yet': '还没有应用',
  'Application Name': '应用名称',
  'Homepage URL': '主页地址',
  'Logo URL': '图标地址',
  'Redirect URIs': '回调地址',
  'One per line. Must be https, or http on localhost.': '每行一个。必须是 https，本地开发可用 http://localhost。',
  'Requested Scopes': '申请的权限',
  'Public client (no secret, PKCE only)': '公开客户端（无密钥，仅 PKCE）',
  'Client Secret': '客户端密钥',
  'Copy it now — it will not be shown again.': '请立即复制，此后不再显示。',
  'Rotate Secret': '重置密钥',
  'Rotating the secret will break any deployment still using the old one. Continue?': '重置密钥会使仍在使用旧密钥的部署立即失效，确定继续？',
  'Delete this application? Every user session it holds will end.': '删除该应用？它持有的所有用户会话将立即结束。',
  'Authorized Applications': '已授权的应用',
  'No authorized applications': '还没有授权任何应用',
  'Revoke Access': '撤销授权',
  'Revoke access for this application? It will lose its tokens immediately.': '撤销该应用的授权？它持有的令牌将立即失效。',
  'Authorized on': '授权于',
  'Draft': '草稿',
  'Active': '已上线',
  'Application disabled': '已禁用',
  'Reviewed': '已审核',
  'Not reviewed': '未审核',
  'Disable': '禁用',
  'Disable this application? Every token it holds will be revoked.': '禁用该应用？它持有的所有令牌将被吊销。',
  'Mark Reviewed': '标记为已审核',
  'Unmark Reviewed': '取消已审核',
  'Active tokens': '有效令牌',
  'Authorizations': '授权数',
  'Description': '简介',
  'Search': '搜索',
  '-- Select --': '-- 请选择 --',
  'Text': '文本',
  'File': '文件',
  'Replace': '更换',
  'Upload File': '上传文件',
  'Uploading': '上传中…',
  'Upload failed': '上传失败',
  'Forms': '表单',
  'My Forms': '我创建的表单',
  'Forms I Filled': '我填写的表单',
  'Create Form': '创建表单',
  'Edit Form': '编辑表单',
  'Form Title': '表单标题',
  'Form Description': '表单说明',
  'Untitled Form': '未命名表单',
  'Published': '已发布',
  'Questions': '问题',
  'Responses': '回复',
  'Delete Form': '删除表单',
  'Are you sure you want to delete this form?': '确定删除这个表单吗？已提交的回复会一并删除。',
  'Delete': '删除',
  'Preview': '预览',
  'Submit': '提交',
  'Update Response': '更新回复',
  'Edit Response': '修改回复',
  'Response recorded': '已记录你的回复',
  'You already responded. Submitting again replaces your previous answers.': '你已填写过，再次提交会覆盖之前的回答。',
  'No forms yet': '还没有创建过表单',
  'No responses yet': '还没有回复',
  'You have not filled in any form yet': '还没有填写过表单',
  'Last updated': '最后更新',
  'This form is not accepting responses': '该表单暂不接受回复',
  'Managed by the event': '活动的报名表单，请在活动页编辑',
  'Cancel Order': '取消订单',
  'Cancel Application': '取消申请',
  'Are you sure you want to cancel this order? You can register again afterwards.': '确定取消这笔订单吗？取消后可以重新报名。',
  'Are you sure you want to withdraw your application? You can apply again afterwards.': '确定撤回申请吗？撤回后可以重新申请。',
  'Application cancelled': '已取消申请',
  'Long Text': '长文本',
  'Public responses': '公开展示所有回复',
  'Public responses intro': '任何人都能看到全部回答，以及回答者（昵称和头像，不含邮箱和手机号）。默认关闭；关掉后重新隐藏。',
  'View responses': '查看回复',
  'Edit my answers': '修改我的回答',
  'My Application': '我的报名表',
  'Date': '日期',
  'Multi Select': '多选',
  'Download CSV': '下载 CSV',
  'Submitted at': '提交时间',
  'Name': '姓名',
  'My Response': '我的回复',
  'Discussion': '讨论',
  'Topics': '主题',
  'All Boards': '全部板块',
  'New Topic': '发布主题',
  'Post Topic': '发布',
  'Reply': '回复',
  'Replies': '回复',
  'No topics yet': '还没有主题',
  'No replies yet': '还没有回复',
  'Be the first to post': '来发第一帖',
  'Write a reply': '写下你的回复',
  'Pinned': '置顶',
  'Locked': '已锁定',
  'Hidden': '已隐藏',
  'This topic is locked': '该主题已锁定，不能再回复',
  'This topic is hidden': '该主题已被隐藏，只有你和管理员可见',
  'This reply is hidden': '该回复已被隐藏，只有你和管理员可见',
  'Reply deleted': '回复已删除',
  'Reply hidden': '回复已被隐藏',
  'In reply to': '回复',
  'Last reply': '最后回复',
  'Topic Title': '标题',
  'Topic Body': '正文',
  'Board': '板块',
  'Only visible to {1}': '仅 {1} 可见',
  'Members of this group': '本社群成员',
  'Managers of this group': '本社群管理员',
  'Invited people only': '受邀成员',
  'Sign in to post': '登录后参与讨论',
  'Pin': '置顶',
  'Unpin': '取消置顶',
  'Lock': '锁定',
  'Unlock': '解锁',
  'Hide': '隐藏',
  'Unhide': '取消隐藏',
  'Reason (optional)': '原因（可选）',
  'Latest': '最近活跃',
  'Newest': '最新发布',
  'Unanswered': '无人回复',
  'Load More': '加载更多',
  'Delete this permanently?': '确定删除？删除后所有人都将看不到它。',
  'Hide this topic': '隐藏该主题',
  'Hide this reply': '隐藏该回复',
  'The author will still see it, along with this reason.': '作者仍然可以看到它，以及你填写的原因。',
  'Teams': '团队',
  'Team': '团队',
  'New Team': '新建团队',
  'Team Name': '团队名称',
  'Team Members': '团队成员',
  'Add Member': '添加成员',
  'No teams yet': '还没有团队',
  'Delete this team?': '确定删除该团队？成员仍留在社群里，但仅对该团队可见的讨论板块将只有管理员能看到。',
  'Visible to teams': '可见团队',
  'Show publicly': '对外公开',
  'Owner only': '仅所有者可改',
  'Remove this person from the group?': '将此人移出社群？其团队、角色和相应权限都会随成员身份一并失去。',
}

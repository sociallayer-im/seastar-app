...
export const getMarkersByGroupHandle = async ({params, clientMode}:SolaSdkFunctionParams<{groupHandle: string}>) => {
    ...
    const resp = await fetch(`${apiUrl}/marker/list?group_handle=${encodeURIComponent(params.groupHandle)}`)
    const data = await resp.json()
    return data.markers as Marker[]
}
...
export const getMarkersByGroupHandleAndCategory = async ({params, clientMode}:SolaSdkFunctionParams<{groupHandle: string, category: string}>) => {
    const apiUrl = getSdkConfig(clientMode).api
    const resp = await fetch(`${apiUrl}/marker/list?group_handle=${encodeURIComponent(params.groupHandle)}&category=${encodeURIComponent(params.category)}`)
    const data = await resp.json()
    return data.markers as Marker[]
}
...
export const getMarkerById = async ({params, clientMode}:SolaSdkFunctionParams<{markerId: number}>) => {
    const apiUrl = getSdkConfig(clientMode).api
    const resp = await fetch(`${apiUrl}/marker/get?id=${params.markerId}`)
    const data = await resp.json()
    return (data.marker as MarkerDetail) || null
}
